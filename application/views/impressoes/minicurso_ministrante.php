...
<div class="bg_minicursos">
				...
				<?php
		        if(count($min_minicurso) == 0){
		        	echo 'Nenhuma informação encontrada.';

		        }else{
					foreach ($min_minicurso as $list) {

						$titulo = explode(".", $list->titulo);
						echo '<p class="texto_minicurso">
						Certificamos que <span class="texto_nome">'. strtoupper ($list->nome) .'</span> participou da XVII Semana Universitária, promovida pelo Campus Avançado “Profa. Maria Elisa de Albuquerque Maia” (CAMEAM), da Universidade do Estado do Rio Grande do Norte (UERN), no período de 04 a 07 de outubro de 2017, na condição de ministrante do minicurso <span class="texto_negrito">'. $list->titulo .'</span>, realizado no dia '. retira_dia($list->data) .' de outubro de 2017, perfazendo a carga-horária de <span class="texto_negrito">'. $list->carga_horaria .'</span> horas.
						</p>';
