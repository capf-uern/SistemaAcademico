...
<div class="bg_minicursos">
				...
				<?php
		        if(count($minicurso) == 0){
		        	echo 'Nenhuma informação encontrada.';

		        }else{
					foreach ($minicurso as $list) {

						$titulo = explode(".", $list->titulo);
						echo '<p class="texto_minicurso">
						Certificamos que <span class="texto_nome">'. strtoupper ($list->nome) .'</span> participou do minicurso <span class="texto_negrito">'.  $titulo[0] .'</span>, durante a XVII Semana Universitária, promovida pelo Campus Avançado “Profa. Maria Elisa de Albuquerque Maia” (CAMEAM), da Universidade do Estado do Rio Grande do Norte (UERN), no período de 04 a 07 de outubro de 2017, perfazendo a carga-horária de <span class="texto_negrito">'. $list->carga_horaria .'</span> horas.
						</p>';
