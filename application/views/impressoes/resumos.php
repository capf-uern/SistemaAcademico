...
<div class="bg_minicursos">
		        ...
		        if(count($resumos) == 0){
					...
					foreach ($resumos as $list) {

						$titulo = explode(".", $list->titulo);
						echo '<p class="texto_minicurso">
						Certificamos que <span class="texto_nome">'. strtoupper ($list->nome) .'</span> participou da XVII Semana Universitária, promovida pelo Campus Avançado “Profa. Maria Elisa de Albuquerque Maia” (CAMEAM), da Universidade do Estado do Rio Grande do Norte (UERN), na condição de apresentador do trabalho <span class="texto_negrito">'. $list->titulo .'</span>.
						</p>';
